...
import { industries, bundles, bundleBaseline } from "@/lib/content";
  ...
  return (
    <>
      <PageHero
        ...
        subtitle="Every market is different. We adapt strategy, channels, and execution to the realities of how your industry actually buys and grows — with a ready-made service bundle for each."
      />
      ...
      <section className="px-5 pt-10 sm:px-8">
        <div className="mx-auto max-w-[1240px] rounded-2xl border border-[rgba(168,85,247,0.25)] bg-[linear-gradient(160deg,rgba(30,20,55,0.7),rgba(12,10,24,0.7))] p-6 sm:p-8">
          <div className="eyebrow mb-2.5">Included in Every Bundle</div>
              ...
              <div key={i.title} className="card-i flex flex-col">
                ...
                {bundle && (
                  <div className="mt-5 border-t border-[var(--color-line)] pt-4">
                    <div className="mb-1 text-[11.5px] font-semibold uppercase tracking-[0.09em] text-[var(--color-brand-soft)]">Recommended Bundle</div>
                    <Link href={`/bundles/${bundle.slug}`} className="mb-1.5 inline-flex items-center gap-1.5 text-[14.5px] font-semibold text-white hover:text-[var(--color-brand-soft)]">
                      {bundle.name} <Icon name="arrow_forward" className="text-[15px]" />
                    </Link>
                    <p className="mb-3 text-[12.5px] leading-[1.55] text-[var(--color-faint)]">{bundle.pitch}</p>
                    <div className="flex flex-wrap gap-1.5">
                      {bundle.items.map((s) => (
                        <Link key={s.label} href={`/services/${s.slug}`} className="rounded-full border border-[rgba(168,85,247,0.25)] bg-[rgba(124,58,237,0.12)] px-2.5 py-1 text-[11.5px] font-medium text-[var(--color-brand-soft)] hover:border-[rgba(168,85,247,0.5)] hover:text-white">
                          {s.label}
                        </Link>
                      ))}
                    </div>
